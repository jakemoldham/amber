...
module Amber::Router
  class Cookies
    ...
    module ChainedCookieStore
      def permanent
        @permanent ||= PermanentCookieStore.new(self, @key_generator, @options)
      end

      def encrypted
        @encrypted ||= EncryptedCookieStore.new(self, @key_generator, @options)
      end
    end

    class CookieStore
      include Enumerable(String)
      include ChainedCookieStore
      ...
      def self.build(request, key_generator = Amber::Support::KeyGenerator.new("secret"))
    ...
    class PermanentCookieStore # :nodoc:
      include ChainedCookieStore

      def initialize(parent_jar : CookieStore, key_generator, options = Hash(String, String))
    ...
    class EncryptedCookieStore # :nodoc:
      include ChainedCookieStore
      include SerializedCookieStore
      ...
      def initialize(parent_jar, key_generator, options = {} of String => String)
        ...
        @encryptor = Amber::Support::MessageEncryptor.new(secret, sign_secret: sign_secret, digest: digest)
      end
      ...
      def set(name : String, value : String, path : String = "/", expires : Time? = nil, domain : String? = nil, secure : Bool = false, http_only : Bool = false, extension : String? = nil)
        ...
        raise Amber::Exceptions::CookieOverflow.new if cookie.value.bytesize > MAX_COOKIE_SIZE
        @parent_jar[name] = cookie
      end

      private def decrypt_and_verify(encrypted_message)
        String.new(@encryptor.decrypt_and_verify(encrypted_message))
      rescue e : Amber::Exceptions::InvalidSignature | Amber::Exceptions::InvalidMessage
        "{\"value\":\"\"}"
      end
    end
  end
